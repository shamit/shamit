## shambrain
[![Build Status](https://travis-ci.org/uwescience/shambrain.svg?branch=master)](https://travis-ci.org/uwescience/shambrain)
...
repository into your computer and follow the instructions at the [bottom of this page](#using-shambrain-as-a-template).
...
`shambrain/shambrain.py`. This code provided in this file is intentionally rather
...
We place the module code in a file called `shambrain.py` in directory called
`shambrain`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import shambrain as sb` in an
interactive Python session, the classes and functions defined inside of the
`shambrain.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the shambrain module, the main classes defined at the bottom of
...
Either way, you can create a `shambrain/data` folder in which you can
...
(typically, something like our `shambrain/tests/test_shambrain.py`). Within each
...
have one such test in `shambrain/tests/test_shambrain.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/shambrain`),
and type:
...
will see a message such as:
    ...
    shambrain/tests/test_shambrain.py .F...
    ...
    shambrain/tests/test_shambrain.py:49: AssertionError
...
To document `shambrain` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/shambrain/blob/master/doc/_static/RTD-advanced-conf.png)

 http://shambrain.readthedocs.org/en/latest/
...
A `shambrain/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/shambrain) for the software. This
...
`shambrain` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `shambrain`, we use the
...
[here](https://github.com/uwescience/shambrain/blob/master/scripts/Figure1.ipynb).


### Using `shambrain` as a template
...
computer under the name you will want your project to have:
	...
	git clone https://github.com/uwescience/shambrain smallish
	cd smallish
...
Next, you will want to move `shambrain/shambrain.py` to be called
`smallish/smallish.py`

	git mv shambrain smallish
	git mv smallish/shambrain.py smallish/smallish.py
	git mv smallish/tests/test_shambrain.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `shambrain` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `shambrain` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `shambrain` in them. Type
the following to see where all these files are:

	git grep shambrain

You can replace `shambrain` for `smallish` fastly with:

	git grep -l 'shambrain' | xargs sed -i 's/shambrain/shambrain/g'
...
Edit all the mentions of `shambrain` in the `shambrain/__init__.py` file, and in
the `shambrain/version.py` file as well.
